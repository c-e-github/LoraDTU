    ...
    if (_allMode == LedState_t::On) {
        ...
        if (Hoymiles.getNumInverters() && Datastore.getIsAtLeastOnePollEnabled()) {
            ...
            if (Datastore.getIsAllEnabledReachable() && Datastore.getIsAllEnabledProducing()) {
                _ledMode[1] = LedState_t::Off; // wenn alles ok
            }
            if (Datastore.getIsAllEnabledReachable() && !Datastore.getIsAllEnabledProducing()) {
                //_ledMode[1] = LedState_t::Blink;
                _ledMode[1] = LedState_t::Blink; // wenn verbunden, aber nix produziert wird -> ausgeschaltet
            }
            if (!Datastore.getIsAllEnabledReachable()) {
                _ledMode[1] = LedState_t::On; // wenn keine kommunikation mit hoymiles
            }
        }
