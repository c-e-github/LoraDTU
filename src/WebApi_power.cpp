...
void WebApiPowerClass::init(AsyncWebServer& server)
{
    ...
    _server->on("/toggleDisplay", HTTP_POST, std::bind(&WebApiPowerClass::toggledisplay, this, _1)); 
    _server->on("/toggleLora", HTTP_POST, std::bind(&WebApiPowerClass::togglelora, this, _1)); 
}
...
void WebApiPowerClass::toggledisplay(AsyncWebServerRequest* request)
{
   displayPowerSave = !displayPowerSave; // Toggle displayPowerSave 
   // Send a JSON response back to the client
   // String jsonResponse = "{\"displaystatus\": " + String(displayPowerSave) + "}";
   // request->send(200, "application/json", jsonResponse);
}

void WebApiPowerClass::togglelora(AsyncWebServerRequest* request)
{
   LoraOn = !LoraOn; // Lora-Verarbeitung an- bzw. ausschalten
   // Send a JSON response back to the client
   // String jsonResponse = "{\"lorastatus\": " + String(LoraOn) + "}";
   // request->send(200, "application/json", jsonResponse);
}
